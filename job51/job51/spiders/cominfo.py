...
class CominfoSpider(CrawlSpider):
    ...
    rules = (
        ...
        #Beijing Rule(LinkExtractor(allow=('http://search\.51job\.com/list/010000,000000,0000,00,9,99,%25E5%25A4%25A7%25E6%2595%25B0%25E6%258D%25AE,2,2\.html\?lang=c&stype=1&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=99&companysize=99&lonlat=0%2C0&radius=-1&ord_field=0&confirmdate=9&fromType=1&dibiaoid=0&address=&line=&specialarea=00&from=&welfare=')),follow=True),
        Rule(LinkExtractor(allow=('http://search\.51job\.com/list/230300,000000,0000,00,9,99,%25E5%25A4%25A7%25E6%2595%25B0%25E6%258D%25AE,2,[0-9]*\.html\?lang=c&stype=1&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=99&companysize=99&lonlat=0%2C0&radius=-1&ord_field=0&confirmdate=9&fromType=1&dibiaoid=0&address=&line=&specialarea=00&from=&welfare=')),follow=True),
        # time.sleep(0.3),
        # 提取匹配 'item.php' 的链接并使用spider的parse_item方法进行分析
        Rule(LinkExtractor(allow=('http://jobs.51job.com/all/co[0-9]*.html')), callback='parse_item'),
    )
